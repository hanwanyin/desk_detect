...
from zai import ZhipuAiClient  # 修改点：使用 zai-sdk
...
# --- 1. 配置与初始化 ---
load_dotenv()
st.set_page_config(page_title="Desk Detective | 桌面神探", page_icon="🕵️", layout="centered")
...
def analyze_image(base64_str):
    try:
        # 修改点：使用与 zai-sdk 兼容的调用方式，原参数格式保持不变（与智谱官方 API 一致）
        response = client.chat.completions.create(
            model="glm-4v-flash",  # 可替换为 glm-4.6v-flash 等
            messages=[
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("🕵️‍♂️ Desk Detective v2.0 | 科技侧写")
...
if img_file:
    ...
    if st.button(" 开始通灵分析 / Begin Profiling", key="analyze"):
        ...
        lottie_loading = load_lottieurl("https://assets9.lottiefiles.com/packages/lf20_p8bfn8sw.json")
        if lottie_loading and LOTTIE_AVAILABLE:
            with anim_placeholder:
                st_lottie(lottie_loading, speed=1, height=200, key=f"lottie_{time.time()}")
        else:
            anim_placeholder.info("正在扫描桌面灵魂... / Scanning soul...")
        ...
        if result:
            ...
            with col1:
                ...
                radar_placeholder.plotly_chart(generate_radar_chart(result.get('rpg_stats', {})), use_container_width=True)
                time.sleep(0.3)
            with col2:
                ...
                """, unsafe_allow_html=True)
                mbti_placeholder = st.empty()
                mbti_placeholder.markdown(f'<div class="mbti-stamp">{result.get("mbti_desk", "UNKN")}</div>', unsafe_allow_html=True)
            time.sleep(0.5)

            roast_placeholder = st.empty()
            roast_placeholder.markdown(f"""
